...
class SplitterService {
  ...
  private def getPolicyFromRows(def rows){
    def insurance = Insurance.findByNameIlike("%${rows[2].getCell(0).stringCellValue}%")
  ...
  private def getPaymentMethodFromRow(XSSFRow row){
    ...
    def addBankOrCardProvider = [(BankAccount.class.simpleName):{ paymentMethod,bankName -> paymentMethod.bank = Bank.findByName(bankName ?: "") },
